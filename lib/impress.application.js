...
impress.mixinApplication = function(application) {
  ...
  function checkTemplate(templateName, templatePath) {
    console.log('---2---'+templatePath);
    api.fs.exists(templatePath, function(exists) {
      if (exists) application.systemTemplates[templateName] = '/templates/' + templateName + '.template'; // templatePath;
    });
  }
  ...
  application.createScript = function(fileName, callback) {
    ...
    else api.fs.readFile(application.dir + fileName, function(err, code) {
      if (err) {
  ...
  application.preloadDirectory = function(relPath, depth, callback) {
    ...
    var absPath = api.impress.addTrailingSlash(application.appDir) + relPath;
      ...
      if (err) {
        ...
        api.async.each(files, function(fileName, cb) {
          var fileExt = api.impress.fileExt(fileName),
              filePath = fileName; // api.impress.addTrailingSlash(absPath) + fileName; //!!!
        ...
        application.watchCache(api.impress.addTrailingSlash(absPath));
  ...
  application.loadPlaces = function(callback) {
    ...
    application.watchCache(application.modelDir + '/');
  };
    ...
    api.fs.exists(placeDir, function(exists) {
      if (exists) {
        ...
        if (!api.impress.inArray([ 'setup', 'init' ], placeName)) {
          application.watchCache(api.impress.addTrailingSlash(placeDir));
        }
      } else callback();
    });
  };
  ...
  application.loadPlaceFile = function(placeName, placeDir, file, callback) {
    var sectionName = api.path.basename(file, '.js');
    application.createScript('/' + placeName + '/' + file, function(err, exports) {
  ...
  application.compress = function(filePath, stats, callback) {
    console.log('application.compress('+filePath+', stats, callback), '+application.dir+ '/static' +filePath);
    api.fs.readFile(application.dir + '/static' + filePath, function(error, data) {
      console.log('@---1');
      if (error) {
        console.log('@---2');
  ...
  application.handler = function(method, path, handler, meta) {
    var dirPath = application.appDir + api.impress.addTrailingSlash(path),
  ...
  application.updateFileCache = function(filePath, stats) {
    api.fs.exists(application.dir + filePath, function(exists) {
      if (!exists) application.cliearDirectoryCache(filePath);
        ...
        if (filePath in application.cache.static) {
          ...
          application.createScript(filePath, function(err, exports) {
            ...
            if (api.impress.startsWith(filePath, application.modelDir)) {
              //
            } else if (api.impress.startsWith(filePath, application.configDir)) {
              // Reload config
              application.config[sectionName] = exports;
  ...
  application.updateCache = function() {
    ...
    api.async.each(updates, function(filePath, callback) {
      api.fs.exists(application.dir + filePath, function(exists) {
        if (exists) {
          api.fs.stat(application.dir + filePath, function(err, stats) {
  ...
  application.watchCache = function(filePath) {
    ...
    if (char === '/' || char === '\\') path = filePath;
    else path = api.impress.stripTrailingSlash(api.path.dirname(filePath));
    if (application) {
      watcher = application.cache.watchers[path];
      if (!watcher) {
        api.fs.exists(application.dir + path, function(exists) {
          if (exists) {
            watcher = api.fs.watch(application.dir + path, function(event, fileName) {
