...
Client.prototype.include = function(data, filePath, cursor, callback) {
  ...
  if (cache) {
    ...
    console.log('---1---' + application.dir + filePath);
...
Client.prototype.static = function(callback) {
  ...
  if (client.path === '/') callback();
    ...
    console.log('Client.prototype.static(1)'+application.staticDir+client.path);
    ...
    if (buffer) {
      ...
      console.log('---4---'+application.staticDir + filePath);
      if (err) {
        console.log('---5');
        ...
        console.log('---6 stats.size='+stats.size+' cacheMaxFileSize='+application.config.files.cacheMaxFileSize);
        var sinceTime = client.req.headers['if-modified-since'];
