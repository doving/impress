...
Client.prototype.fileHandler = function(handler, inheritance, callback) {
  ...
  if (fileCache) {
    ...
    if (exists) {
      ...
      if (!inheritance) {
        console.log('!!! fileOriginal='+fileOriginal+', filePath='+filePath);
      ...
      if (client.execPath !== '/' && handler !== 'meta') {
        ...
        fileOriginal = client.pathDir + fileName;
        console.log('!!! fileOriginal='+fileOriginal+', impress.FILE_NOT_FOUND');
...
Client.prototype.template = function(data, file, cursor, callback) {
  ...
  api.async.detectSeries(files, function(item, callback) {
    ...
    if (fileCache) {
      ...
      console.log('!!! save template fileOriginal='+fileOriginal+', filePath='+filePath);
      application.cache.files[fileOriginal] = filePath;
      application.watchCache(fileOriginal);
    } else {
      ...
      if ((client.execPath !== '/') && (client.execPath !== '.')) {
        ...
        fileOriginal = client.execPathDir + fileName;
        console.log('!!! save template fileOriginal='+fileOriginal+', impress.FILE_NOT_FOUND');
...
Client.prototype.static = function(callback) {
  ...
  if (client.path === '/') callback();
    ...
    } else api.fs.stat(application.staticDir + filePath, function(err, stats) {
      if (err) {
