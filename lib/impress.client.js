...
Client.prototype.dispatch = function() {
  var client = this;
  console.log('Client.prototype.dispatch');
...
Client.prototype.restoreSession = function(callback) {
  console.log('Client.prototype.restoreSession');
  var client = this,
      application = client.application;
...
Client.prototype.processing = function() {
  console.log('Client.prototype.processing');
  var client = this,
      application = client.application;

  client.detectRealPath(function() {
    console.log('Client.prototype.processing '+client.realPath);
    var cache = application.cache.pages[client.realPath];
    if (cache && (client.startTime < cache.expireTime)) {
      console.log('Client.prototype.processing 1');
      ...
      console.log('Client.prototype.processing 2');
      ...
      if (client.typeExt === 'sse') client.eventChannel = null;
        ...
        console.log('Client.prototype.processing 3');
        console.dir(Object.keys(client));
        console.dir(Object.keys(client.res));
        if (contentType) client.res.setHeader('Content-Type', contentType);
        console.log('Client.prototype.processing 3.1');
        var allowOrigin = api.impress.getByPath(application.config, 'application.allowOrigin');
        console.log('Client.prototype.processing 3.2');
        if (allowOrigin) {
          console.log('Client.prototype.processing 3.3');
      ...
      //if (application.sse) application.sse.initialize(client);

      console.log('Client.prototype.processing 4');
      // Execute handlers
      api.async.eachSeries(handlers, function(handler, callback) {
        ...
        console.log('Client.prototype.processing 5');
        if (!err && !client.res.headersSent) {
          console.log('Client.prototype.processing 6');
          if (client.access.allowed) {
            console.log('Client.prototype.processing 7');
            if (client.access.auth && !client.local) {
              console.log('Client.prototype.processing 8');
...
Client.prototype.inherited = function(callback) {
  ...
  if (client.execPath !== '/' && client.currentHandler !== 'meta') {
    client.execPath = api.impress.addTrailingSlash(api.path.dirname(client.execPath));
...
Client.prototype.fileHandler = function(handler, inheritance, callback) {
  ...
  } else api.fs.exists(application.dir + filePath, function(exists) {
    var fileOriginal;
    if (exists) {
...
Client.prototype.detectRealPath = function(callback) {
  ...
  console.log('Client.prototype.detectRealPath '+application.dir + client.realPathDir);
  if ((folderCache && folderCache !== impress.FILE_NOT_FOUND) || client.realPath === '/') callback();
  else api.fs.exists(application.dir + client.realPathDir, function(exists) {
    if (exists) {
      application.cache.folders[client.realPath] = impress.FILE_EXISTS;
      callback();
    } else {
      client.realPath = api.impress.addTrailingSlash(api.path.dirname(client.realPath));
...
Client.prototype.template = function(data, file, cursor, callback) {
  ...
  api.async.detectSeries(files, function(item, callback) {
    ...
    else api.fs.exists(application.dir + filePath, callback);
  }, function(result) {
    var fileOriginal;
    if (fileCache) {
      if (fileCache !== impress.FILE_NOT_FOUND) client.include(data, fileCache, cursor, callback);
      else callback(impress.TPL_NOT_FOUND + file + '!1');
    } else if (result) {
      ...
      if ((client.execPath !== '/') && (client.execPath !== '.')) {
        client.execPath = api.impress.addTrailingSlash(api.path.dirname(client.execPath));
        client.execPathDir = '/app' + client.execPath; // application.appDir + client.execPath;
        ...
        application.watchCache(fileOriginal);
        callback(impress.TPL_NOT_FOUND + file  + '!2');
      }
    }
  });
};
...
Client.prototype.include = function(data, filePath, cursor, callback) {
  ...
  if (cache) {
    ...
    api.fs.readFile(application.dir + filePath, 'utf8', function(err, tpl) {
      if (err) callback(impress.TPL_NOT_FOUND + filePath + '!3');
      else {
      ...
      if (item.type === 'plain') {
        ...
        client.template(data, item.name, cursorNew, function(tpl) {
          if (tpl === impress.FILE_IS_EMPTY) callback();
            ...
            result += tpl || impress.TPL_NOT_FOUND + item.name + '!4';
            callback();
          }
        });
      }
...
Client.prototype.static = function(callback) {
  ...
  if (client.path === '/') callback();
    ...
    var filePath = client.path, //api.impress.stripTrailingSlash(application.staticDir + client.path),
    ...
    if (buffer) {
      ...
      console.log('static '+application.staticDir + filePath);
      api.fs.stat(application.staticDir + filePath, function(err, stats) {
      if (err) {
        console.log('---1');
        ...
        console.log('---2');
        var sinceTime = client.req.headers['if-modified-since'];
        if (sinceTime && api.impress.isTimeEqual(sinceTime, stats.mtime)) {
          console.log('---3');
          client.error(304);
        } else if (stats.isDirectory()) {
          console.log('---4');
          var appPath = api.impress.stripTrailingSlash(application.appDir + client.path);
          api.fs.stat(application.dir + appPath, function(err, stats) {
            console.log('---5');
