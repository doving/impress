...
Client.prototype.download = function(filePath, attachmentName, callback) {
  ...
  api.fs.stat(application.dir + filePath, function(err, stats) {
    if (err) {
      ...
      var stream = api.fs.createReadStream(application.dir + filePath);
    ...
    api.mkdirp(targetDir, function(err) {
      ...
      fd.on('end', function() {
        ...
        if (data.compressionFlag === 'Z') {
          ...
          arc.pipe(out);
          arc.on('end', function() { saveUploadedFile(data, onFile); });
          ...
          inp.on('end', function() { saveUploadedFile(data, onFile); });
...
Client.prototype.stream = function(filePath, stats) {
  console.log('Client.prototype.stream('+filePath+', stats)');
  var client = this,
      application = client.application;

  console.log('+++'+application.dir + '/static' + filePath);
  ...
  if (range) {
    ...
    stream = api.fs.createReadStream(application.dir + '/static' + filePath, { start: start, end: end });
  } else {
    console.log('+++2');
    client.res.setHeader('Content-Length', stats.size);
    client.res.setHeader('Last-Modified', stats.mtime.toGMTString());
    stream = api.fs.createReadStream(application.dir + '/static' + filePath);
  }
